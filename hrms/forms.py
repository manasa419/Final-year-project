...
class EmployeeForm(forms.ModelForm):
    ...
    gender = forms.ChoiceField(choices=Employee.GENDER_CHOICES, widget=forms.Select(attrs={'class': 'form-control'}))
    ...
    language = forms.ChoiceField(choices=Employee.LANGUAGE_CHOICES, widget=forms.Select(attrs={'class': 'form-control'}))
...
class AttendanceForm(forms.ModelForm):
    status = forms.ChoiceField(choices=Attendance.STATUS_CHOICES, widget=forms.Select(attrs={'class': 'form-control w-50'}))
    staff = forms.ModelChoiceField(
        queryset=Employee.objects.filter(Q(attendance__status=None) | ~Q(attendance__date=timezone.localdate())),
